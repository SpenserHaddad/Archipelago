...
def download_SNI():
    print("Updating SNI")
    machine_to_go = {"x86_64": "amd64", "aarch64": "arm64", "armv7l": "arm"}
...
if os.path.exists("X:/pw.txt"):
    ...
    with open("X:/pw.txt", encoding="utf-8-sig") as f:
        pw = f.read()
    signtool = (
        r'signtool sign /f X:/_SITS_Zertifikat_.pfx /p "' + pw + r'" /fd sha256 /tr http://timestamp.digicert.com/ '
    )
else:
    signtool = None
...
arch_folder = "exe.{platform}-{version}".format(platform=build_platform, version=sysconfig.get_python_version())
...
exes = [
    ...
    for c in components
    if c.script_name and c.frozen_name
]
...
class BuildExeCommand(cx_Freeze.command.build_exe.BuildEXE):
    ...
    def run(self):
        ...
        for path in build_ext.get_outputs():
            ...
            for match in Path().glob(pattern):
                try:
                    ...
                    warnings.warn(
                        f"Could not delete old build output: {match}\n"
        ...
        shutil.copytree(
            os.path.join(os.path.dirname(kivy.__file__), "data"), self.buildfolder / "data", dirs_exist_ok=True
        )
        ...
        for worldname, worldtype in AutoWorldRegister.world_types.items():
            if worldname not in non_apworlds:
                ...
                with zipfile.ZipFile(
                    self.libfolder / "worlds" / (file_name + ".apworld"), "x", zipfile.ZIP_DEFLATED, compresslevel=9
                ) as zf:
        ...
        if is_windows:
            ...
            with open("installdelete.iss", "w") as f:
                f.writelines(
                    'Type: filesandordirs; Name: "{app}\\lib\\worlds\\' + world_directory + '"\n'
